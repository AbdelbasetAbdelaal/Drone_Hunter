...
sector_status = '''
        ...
        if is_unlocked:
'''
content = re.sub(r't_mark = .*?if is_completed else \("> " if is_selected else ""\)\s+s_text = font_card\.render.*?if is_unlocked:', sector_status.strip() + '\n            ', content, flags=re.DOTALL, count=1)
...
mission_status = '''
        ...
        if is_completed:
            ...
            canvas.blit(st, (m_rect.right - st.get_width() - 20, my_y + 25))
'''
content = re.sub(r't_mark = .*?if is_completed else ""\s+m_num =.*?canvas\.blit\(st, \(m_rect\.right - 90, my_y \+ 25\)\)', mission_status.strip(), content, flags=re.DOTALL, count=1)
