...
from src.data.settings import (
    ...
    COLOR_OVERCLOCK, COLOR_WHITE, COLOR_HUD
)
...
def draw_hangar_shop_ui(canvas: pygame.Surface, coins: int, current_sector_idx: int, upgrade_levels: dict[str, int]) -> tuple[pygame.Rect, pygame.Rect, dict]:
    ...
    canvas.blit(coin_hdr, (SCREEN_WIDTH - 300, 28))

    items = [
        ("1", "battery", "Reinforced Hull Armor", COLOR_EMERALD),
        ...
        ("U", "overdrive", "Overdrive Reactor Core", COLOR_OVERCLOCK)
    ]

    card_w, card_h = 570, 80
    ...
    for idx, (key_num, upg_id, upg_name, color) in enumerate(items):
        ...
        pygame.draw.rect(canvas, COLOR_WHITE if is_hover else color, card_rect, 2, border_radius=8)
        ...
        canvas.blit(txt_lvl, (cx + 14, cy + 34))

        # Progress bar
        pygame.draw.rect(canvas, (30, 41, 59), (cx + 14, cy + 58, 520, 8), border_radius=3)
        fill_w = int(520 * (lvl / max_lvl))
        if fill_w > 0:
            pygame.draw.rect(canvas, color, (cx + 14, cy + 58, fill_w, 8), border_radius=3)

    # Skin Theme Selector Button
    skin_btn_rect = pygame.Rect(SCREEN_WIDTH - 580, 22, 220, 40)
